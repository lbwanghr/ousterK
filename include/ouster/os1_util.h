...
std::function<void(const uint8_t*, iterator_type it)> batch_to_iter(
	...
	double y = 0;
	double z = 0;
	double MIN_HEIGHT = -1.9f;
	double MAX_HEIGHT = 0.5f;
	double CAR_FRONT = -3.2f;
	double CAR_BACK = 1.5f;
	double CAR_LEFT = -1.0f;
	double CAR_RIGHT = 1.0f;
    ...
    return [=](const uint8_t* packet_buf, iterator_type it) mutable {
        ...
        for (int icol = 0; icol < OS1::columns_per_buffer; icol++) {
            ...
            for (uint8_t ipx = 0; ipx < H; ipx++) {
				...
				/************************filter z to proper range***********************/
				x = r * 0.001f * xyz_lut[ind + 0];
				y = r * 0.001f * xyz_lut[ind + 1];
				z = r * 0.001f * xyz_lut[ind + 2];
				if( z < MIN_HEIGHT || z > MAX_HEIGHT || (x > CAR_FRONT && x < CAR_BACK && y> CAR_LEFT && y<CAR_RIGHT) )
				{
					x=0; y=0; z=0;
				}
				/************************filter x,y,z to proper range***********************/
